...
const statements = {
    ...
    selectEventsSince: db.prepare(`
      ...
      ORDER BY id ASC
    `),
    pruneGames: db.prepare(`
        DELETE FROM games WHERE updated_at < datetime('now', ?)
    `),
    upsertSubscription: db.prepare(`
        INSERT INTO push_subscriptions (player_id, endpoint, p256dh, auth)
        VALUES (@playerID, @endpoint, @p256dh, @auth)
    ...
    `),
    selectSubscriptionsForPlayer: db.prepare(`
...
// ****************************************************************
// delete games (and their moves/scores/players/events via ON DELETE CASCADE)
// that haven't been touched in maxAgeMs, so the db doesn't grow forever
function pruneOldGames(maxAgeMs) {
    return safely('pruneOldGames', function () {
        let cutoffModifier = `-${Math.max(0, Math.floor(maxAgeMs / 1000))} seconds`
        return statements.pruneGames.run(cutoffModifier).changes
    }, 0)
}

// ****************************************************************
// Web Push subscriptions -- one player identity can hold several (one per
// device/browser they've opted in on). The player row is upserted first
// since a subscription can outlive any single game and players.id is a
// foreign key.
const persistSubscription = db.transaction(function (playerID, subscription) {
    statements.upsertPlayer.run({ id: playerID, isBot: 0 })
...
function saveSubscription(playerID, subscription) {
    return safely('saveSubscription', function () {
        persistSubscription(playerID, subscription)
        return true
    }, false)
}

function removeSubscription(endpoint) {
    return safely('removeSubscription', function () {
        statements.deleteSubscriptionByEndpoint.run(endpoint)
        return true
    }, false)
}

function getSubscriptionsForPlayer(playerID) {
    return safely('getSubscriptionsForPlayer', function () {
        return statements.selectSubscriptionsForPlayer.all(playerID)
    }, [])
}
